...
void HttpServer::run(int thread_num, int max_queque_size) {
    ThreadPool threadPool(thread_num, max_queque_size);//创建线程池，设置请求队列中最大请求个数
...
//        threadPool.append(threadTask);
    int epoll_fd = Epoll::init(1024);//创建并返回指向事件表的一个文件描述符，并且创建事件数组
    //std::cout << "a|epoll_fd=" << epoll_fd << std::endl;
//        int ret = setnonblocking(epoll_fd);
//        if (ret < 0) {
//            std::cout << "epoll_fd set nonblocking error" << std::endl;
//        }
    std::shared_ptr<HttpData> httpData(new HttpData());//HttpData()类共享指针
    httpData->epoll_fd = epoll_fd;
    serverSocket.epoll_fd = epoll_fd;   // 之前就是这里忘了添加,导致穿进去的serverSocket具有不正确的epoll_fd

    __uint32_t event = (EPOLLIN | EPOLLET);//事件，在Epoll函数实现中是直接从新设置
    Epoll::addfd(epoll_fd, serverSocket.listen_fd, event, httpData);//给监听socket设置监听事件
...
//
//        test end
        // max_event:监听事件个数   timeout -1 永远阻塞直到有事件
        printf("wait request\n");
        std::vector<std::shared_ptr<HttpData>> events = Epoll::poll(serverSocket, 1024, -1);
        // FIXME 将事件传递给 线程池
        for (auto& req : events) {//传入有数据可读事件和处理函数 将这个事件和处理函数传入请求队列 线程从请求队列中取出事件和处理函数来运行
            //bind函数，this对象绑定到do_requset的第一个占位符上。 返回一个新函数
